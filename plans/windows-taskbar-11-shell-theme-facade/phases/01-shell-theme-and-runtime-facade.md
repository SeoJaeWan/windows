...
| `packages/ui/src/components/common/iconImage/index.tsx` | 교체/정리 | shared runtime leaf도 canonical runtime inventory 일부이며 file map에서 빠질 수 없다. | `common/iconImage/index.tsx`가 `cn()` adoption과 validation coverage를 동시에 가진다. |
...
- current caller-class merge owner inventory가 `cn()` adoption positive signal로 확인되고, `common/iconImage/index.tsx`도 같은 helper owner surface로 전환된다.
...
- input:
  ...
  - 현재 repo context:
    ...
    - `packages/ui/src/components/common/iconImage/index.tsx`는 shared runtime leaf인데도 아직 template-string `className` 결합을 사용한다.
...
- output:
  - 공개 계약:
    ...
    - caller-supplied `className` 또는 유사 merge owner를 가진 current runtime files는 `cn()` adoption positive signal로 확인돼야 하며, `common/iconImage/index.tsx`도 여기 포함된다.
...
- failure/validation: exported/compare-owned family를 anchor에서 읽어도 writable runtime inventory가 이를 커버하지 못하거나, old subset 바깥 backlog를 남긴 채 phase를 닫아야만 하면 이 phase는 실패다.
  - 작업:
    ...
    - `packages/ui/src/components/common/iconImage/index.tsx`를 포함한 shared runtime leaf merge owner를 file map에 드러내고 same helper contract로 전환한다.
...
- 검증:
  ...
  - [ ] `$expected = @('packages/ui/src/components/common/iconImage/index.tsx','packages/ui/src/components/panels/context/contextPanel/index.tsx','packages/ui/src/components/panels/search/searchPanel/index.tsx','packages/ui/src/components/panels/shared/panelSurface/index.tsx','packages/ui/src/components/panels/taskbarContextMenu/index.tsx','packages/ui/src/components/panels/taskbarHoverPreview/index.tsx','packages/ui/src/components/panels/windows/windowsPanel/index.tsx','packages/ui/src/components/taskbar/taskbar/index.tsx','packages/ui/src/components/taskbar/taskbarClock/index.tsx','packages/ui/src/components/taskbar/taskbarIconButton/index.tsx','packages/ui/src/components/taskbar/taskbarSearch/index.tsx','packages/ui/src/components/taskbar/taskbarWindowsButton/index.tsx'); $actual = rg -l -g '!**/storybook/**' -g '!**/*.stories.tsx' -g '!**/*.test.tsx' '\bcn\(' packages/ui/src/components | ForEach-Object { $_ -replace '\\','/' }; $expected | Where-Object { $_ -notin $actual }` 결과가 비어 있어 current caller-class merge owner inventory가 `cn()` replacement-owner signal을 실제로 채택했음을 확인한다.
