...
function FolderChrome({
  ...
  return (
    <>
      {/* Row 1: Tab titlebar */}
      <div className="folder-titlebar flex items-end gap-0 bg-[#F0F2F5] border-b border-[#d6d2ca] select-none h-[30px]">
        {/* Active tab */}
        <div className="folder-tab flex items-center gap-1.5 px-2 h-[30px] bg-white border-t border-l border-r border-[#d6d2ca] -mb-px rounded-t shrink-0 max-w-[200px]">
          {icon && (
      ...
      <div className="folder-toolbar relative flex items-center gap-1 px-2 bg-white border-b border-[#d6d2ca] h-[36px]">
        {/* Nav controls */}
        <div className="flex items-center shrink-0" aria-hidden>
          <button
            type="button"
        ...
        <div className="hidden md:block relative shrink-0">
          ...
          {searchPanelOpen && (
            <div className="folder-search-overlay absolute right-0 top-full z-10 bg-white border border-[#d6d2ca] shadow-sm rounded w-[320px]">
              {onSearchValueChange && (
                <div className="folder-search-input-row px-3 py-2">
                  <input
                    type="text"
                    className="w-full h-7 px-2 text-xs border border-[#d6d2ca] rounded bg-white text-gray-800 placeholder:text-gray-400 focus:outline-none focus:border-blue-500"
                    value={searchValue ?? ""}
  ...
  return (
    ...
    >
      <div className="folder-body flex h-full overflow-hidden">
        ...
        <aside className="folder-sidebar hidden md:flex flex-col w-40 shrink-0 border-r border-[#d6d2ca] bg-[#F1F3F5] overflow-y-auto py-1">
            ...
            return (
              <div key={item.id} className="folder-sidebar-group">
                ...
                <button
                  type="button"
                  className={cn(
                    "folder-sidebar-row w-full flex items-center gap-1 px-2 py-1.5 text-sm text-left cursor-default select-none",
                    isActive
                ...
                >
                  ...
                  >
                    {item.children && isExpanded ? <ChevronDown12Regular /> : <ChevronRight12Regular />}
                  </span>
                  {item.icon && (
                    <span className="inline-flex items-center justify-center w-3.5 h-3.5 shrink-0" aria-hidden>
                      {item.icon}
                    ...
                    {item.children.map((child) => {
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          className={cn(
                            "folder-sidebar-row folder-sidebar-child w-full flex items-center gap-1 pl-6 pr-2 py-1.5 text-sm text-left cursor-default select-none",
                            isChildActive
                              ? "bg-gray-200 text-gray-900 font-medium"
        ...
        <div className="folder-content flex-1 overflow-y-auto p-2">
          <div className="folder-grid grid grid-cols-2 md:grid-cols-3 gap-x-8 gap-y-10">
            {entries.map((entry) => (
              <button
                key={entry.id}
                type="button"
              ...
              >
                ...
                <div className="folder-entry-thumbnail aspect-[5/3] rounded-lg overflow-hidden bg-gray-100 shrink-0 w-full">
                  <img
                ...
                <div className="folder-entry-body flex flex-col pt-4">
