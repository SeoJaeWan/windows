...
- Use `## 요청 대응표` to map the user's request items to the plan scope without replacing them with planner taxonomy.
- Use `## 작업 묶음 지도` to group work by concrete boundaries such as components, hooks, routes, services, or screens.
- Use `## 공개 계약 요약` to expose touched public surfaces before phase ordering.
- Use `## 상태/연동 규칙` when ownership, winner rules, open/close rules, or no-op rules matter.
- Use `## 제외 항목` to show excluded items one row at a time with reason and approval status.
- Use `## 실행 순서` and `## Phase 실행 카드` to explain sequence after the work bundles and public contracts are already visible.
- End `plan.md` with `## 승인 체크리스트`.
