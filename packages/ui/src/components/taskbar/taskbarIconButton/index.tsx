...
function TaskbarIconButton({
  ...
  return (
    ...
    >
      ...
      {status !== "default" && (
        <span
          className={`taskbar-icon-button__indicator absolute bottom-0.75 left-1/2 h-0.75 -translate-x-1/2 rounded-full transition-[width,background-color,opacity] duration-200 ease-out ${status === "active" ? "w-3 bg-taskbar-active opacity-100" : "w-1.5 bg-taskbar-inactive opacity-90"}`}
          aria-hidden="true"
        />
      )}
    </button>
