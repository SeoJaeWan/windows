...
> 이 문서는 Phase 5 compare report가 지적한 drift만 source tree에서 닫는 실행용 상세 계약이다.
...
| 선행조건 | Phase 5가 exact `kind/state` mismatch report를 남겨야 한다. |
...
- compare rerun이 같은 `folder` / `browser` key naming과 Phase 1 baseline provenance를 계속 사용한다.
...
- input:
  - 시나리오: Phase 5 compare report가 canonical 6 state 중 일부 mismatch를 남긴 뒤, maintainer가 same inventory 안에서 visual drift만 수리하려는 경우
  - fix target source:
...
- output:
  ...
  - 내부 기본값:
    ...
    - final pass/fail은 Phase 1 baseline provenance를 계속 인용한다.
  - 중요한 negative output:
