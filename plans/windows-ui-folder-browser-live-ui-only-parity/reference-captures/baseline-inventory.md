...
| 항목 | 값 |
| --- | --- |
| desktop 원본 캡처 viewport | `1280 × 800` |
| desktop taskbar separator y | `y = 750` (window 하단 경계) |
| desktop 결과 crop 치수 | `1280 × 750` — window area only — taskbar excluded |
| mobile 원본 캡처 viewport | `390 × 844` |
| mobile taskbar separator y | `y = 794` (window 하단 경계) |
| mobile 결과 crop 치수 | `390 × 794` — window area only — taskbar excluded |
| crop 방식 | 원본 상단에서 결과 치수만큼 추출 (하단 taskbar strip ~50px 제거) |

모든 baseline PNG는 live seojaewan.com taskbar (Start button, 검색 pill, 폴더 아이콘, 시계)가 포함되지 않는다.
Phase 4 compare script는 taskbar를 추측하거나 별도 masking 없이 PNG를 그대로 diff 대상으로 사용할 수 있다.
...
| 원본 캡처 viewport | `1280 × 800` |
| 결과 PNG 치수 | `1280 × 750` — window area only — taskbar excluded |
| capture date | 2026-04-18 |
...
| source URL | `https://seojaewan.com/blog` |
| 원본 캡처 viewport | `390 × 844` |
| 결과 PNG 치수 | `390 × 794` — window area only — taskbar excluded |
| capture date | 2026-04-18 |
| open-state trigger | none — default closed state |
...
- taskbar (하단 nav bar) — baseline PNG에서 crop으로 제거됨
- desktop 배경 (gradient)
