...
| 선행조건 | `none` |
| 완료 판단 | `Folder` source와 story만 읽어도 window-only 범위, `items.imageSrc`, desktop sidebar/mobile collapse, internal `WindowFrame` boundary가 분명하다. |
...
| `packages/ui/src/components/windows/folder/index.tsx` | 추가 | `Folder` v1 public surface는 pure props 기반이며 `items` body owner를 직접 가진다. `items` media field는 `imageSrc` 하나만 canonical name으로 둔다. | desktop sidebar, mobile collapse, viewport height, content-only scroll, route-agnostic props가 component source에 고정된다. |
| `packages/ui/src/components/windows/storybook/folderReferenceFixtures.ts` | 추가 | canonical inventory는 `desktop-default`, `mobile-collapsed` 두 state만 가진다. | Folder sample data와 state key가 story/compare owner가 다시 쓰기 쉬운 source로 남는다. |
...
| `packages/ui/src/components/windows/folder/folder.stories.tsx` | 추가 | route 이름이나 page wrapper를 public contract처럼 보이게 하면 안 된다. | `Folder` standalone story가 canonical 2 state를 pure props로 보여 준다. |
...
- `items.imageSrc` naming이 fixture와 component 양쪽에서 한 가지 surface로만 등장한다.
...
- failure/validation: `Folder` contract가 `urlLabel`처럼 route 의미가 강한 naming, second media field, public window-control props, route-specific body slot으로 다시 열리면 blocker다.
- 작업:
