...
| --- | --- | --- | --- |
| Phase 1. canonical inventory와 runtime facade | `packages/ui/src/components/**` runtime inventory 전체를 shell facade + `cn()` 기준으로 다시 묶고, `common/**`를 포함한 exported family 전부를 runtime cleanup boundary에 포함한다. | runtime writable surface가 더 이상 subset family list에 묶이지 않고, `common/**`·exported family·compare-owned family 전부가 같은 shell facade cleanup contract를 공유한다. | Storybook이 따라야 할 canonical runtime surface, `cn()` replacement-owner adoption 상태, scope anchor parity |
...
| `packages/ui/src/components/common/**` | 교체/정리 | shared runtime leaf도 same canonical inventory에서 `cn()` contract를 따른다. | `common/iconImage/index.tsx`가 file map과 validation에서 동시에 coverage를 가진다. |
...
- 이전 상태: plan summary는 taskbar/shared/windows/search subset만 다루는 것처럼 보였고, `common/**`를 포함한 public export와 compare-owned family 일부는 boundary 밖에 남아 있었다. runtime inventory에는 raw shared `var(...)`, `--panel-border` bridge, 문자열 `className` 결합이 subset 안팎에 함께 남아 있었다.
- 이후 상태: runtime cleanup boundary가 `packages/ui/src/components/**` canonical inventory 전체를 기준으로 닫히고, `common/**`까지 포함한 shared leaf/runtime family가 같은 file map 안에 들어온다. supporting owner 파일은 이 inventory를 뒷받침하는 adjacent boundary로만 남고, exported family와 compare-owned family는 read-only anchor를 통해 같은 runtime contract에 포함된다.
- 완료 조건: `packages/ui/src/components/**` runtime inventory에서 explicit negative scope를 제외한 파일에 raw shared var bridge, old shared surface, 문자열 `className` 결합이 남지 않는다. 현재 caller-class merge owner inventory가 `cn()` adoption positive signal로 확인되고, `packages/ui/src/index.ts`와 `compareRoot.tsx` 기준으로 in-scope family가 plan boundary와 모순되지 않는다.
...
- 이전 상태: Storybook cleanup boundary가 taskbar/windows/search subset으로만 적혀 있었고, 실제 `style`/`<style>` owner inventory에는 `context`, `taskbarContextMenu`, `taskbarHoverPreview`, host-composition stories가 더 남아 있었다. thin compare wrapper 사용 패턴도 `CompareLeafStage`를 포함한 실제 repo usage와 어긋나 있었다.
...
- 완료 조건: full-tree story/reference inventory에서 `style`/`<style>` owner 목록이 explicit exception owner와 정확히 일치한다. allowlisted 파일 안에도 raw shared color/border var bridge가 남지 않고, `CompareLeafStage`를 포함한 thin compare wrapper와 reference owner signal은 positive check로 유지된다.
...
| `.claude/rules/storybook.md` | 보강 | `CompareLeafStage`를 포함한 thin compare wrapper, bounded exception owner, host-composition 예외 규칙이 현재 repo 패턴과 맞춰진다. | CompareRoot만 직접 언급하는 낡은 규칙이 사라진다. |
...
- 이후 상태: `.claude` 운영 문서가 `packages/ui/src/components/**` canonical inventory, read-only anchor, explicit negative scope, `CompareLeafStage`를 포함한 Storybook bounded exception taxonomy를 같은 기준으로 안내한다.
- 완료 조건: `.claude` 문서만 읽어도 package-wide cleanup work에서 canonical inventory와 validation alignment rule, `CompareLeafStage`를 포함한 thin compare wrapper owner, host-composition bounded exception rule을 재현할 수 있다.
